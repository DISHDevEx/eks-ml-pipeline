...
def read_raw_input_data(bucket_name_raw_data, folder_name_raw_data, input_year, input_month, input_day, input_hour, rec_type, input_setup):
    """
    inputs
    ------
...
def rec_type_ad_preprocessing(input_df, input_feature_group_name, input_feature_group_version, rec_type):
    """
    inputs
    ------
            input_df: pyspark df
            raw pyspark dataframe to perform processing on
    ...
    if len(input_df.columns) < 3:
        ...
        if rec_type == 'Node':
            rec_type_df = input_df.select("Timestamp", "InstanceId", *processed_features)
        elif rec_type == 'Container':
            rec_type_df = input_df.select("Timestamp",
            ...
            rec_type_df = input_df.select("Timestamp", get_json_object(col("kubernetes"), "$.pod_id").alias("pod_id"),
